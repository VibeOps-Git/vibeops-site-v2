...
import {
  CONFIG, DAILY_REPORT_DIR, resolveDate, hasFlag, listSnapshotDates, loadSnapshot,
...
if (deploys?.data.days_since_deploy != null) {
  facts.push(`Last production deploy: ${deploys.data.latest_production_deploy?.slice(0, 10)} (${deploys.data.days_since_deploy}d ago) — "${deploys.data.deployments?.[0]?.commit_message ?? 'n/a'}".`);
}
...
if (tech) {
  if (!tech.data.issues.length) {
    ...
    for (const i of tech.data.issues) {
...
| Deployments | ${deploys ? date : 'n/a'} | ${deploys ? `last production deploy ${deploys.data.days_since_deploy}d ago` : '_not collected_'} |
| SERP probe (benchmark, frozen 41) | ${serp?.status === 'ok' ? serp.represents.end : '—'} | ${serp?.status === 'ok' ? `${serp.data.summary.in_top_20}/${serp.data.summary.measured} top 20 · ${serp.data.summary.in_top_10} top 10 · ${serp.data.summary.in_top_3} top 3` : '_not collected — needs the attached Chrome_'} |
